...
For anything touching Anthropic/Claude — cost controls, rate limits, spend caps, streaming, prompt caching, security, model selection, usage tracking, etc. — always check the official Anthropic docs for existing best practices and built-in mechanisms before building a custom solution. Anthropic often provides a server-enforced or recommended approach that is more robust than a hand-rolled one; surface it even if the user asked for a custom implementation.
